...
const METAS_MINIMAS_HEADERS = ['EQUIPE', 'COD', 'AREA', 'FORNECEDOR', 'VALOR']
const CSV_DELIMITER = ';'
...
function parseMotorBase(text) {
  ...
  const headerParts = parseCsvLine(headerLine, CSV_DELIMITER).map((h) => String(h || '').trim().toUpperCase())
  const expected = MOTOR_HEADERS.join(CSV_DELIMITER)
  const current = headerParts.join(CSV_DELIMITER)

  if (headerParts.length !== MOTOR_HEADERS.length || current !== expected) {
    throw new Error(
      `Cabecalho invalido. Esperado: ${expected} | Recebido: ${current || '(vazio)'} | Separador: "${CSV_DELIMITER}"`,
    )
  }
  ...
  for (let i = 1; i < nonEmptyLines.length; i += 1) {
    ...
    const values = parseCsvLine(line, CSV_DELIMITER)
...
function parseMetasMinimas(text) {
  const lines = text
    .split(/\r?\n/)
    .map((l, idx) => (idx === 0 ? l.replace(/^\uFEFF/, '') : l))
  ...
  const headerLine = nonEmptyLines[0]
  const headerParts = parseCsvLine(headerLine, CSV_DELIMITER).map((h) => String(h || '').trim().toUpperCase())
  const expected = METAS_MINIMAS_HEADERS.join(CSV_DELIMITER)
  const current = headerParts.join(CSV_DELIMITER)

  if (headerParts.length !== METAS_MINIMAS_HEADERS.length || current !== expected) {
    throw new Error(
      `Cabecalho invalido. Esperado: ${expected} | Recebido: ${current || '(vazio)'} | Separador: "${CSV_DELIMITER}"`,
    )
  }
  ...
  for (let i = 1; i < nonEmptyLines.length; i += 1) {
    ...
    if (values.length < METAS_MINIMAS_HEADERS.length) {
      errors.push(`Linha ${lineNumber}: quantidade de colunas invalida.`)
      continue
    }
    ...
    const row = {
      EQUIPE: String(values[0] ?? '').trim(),
      COD: String(values[1] ?? '').trim(),
      AREA: String(values[2] ?? '').trim(),
      FORNECEDOR: String(values[3] ?? '').trim(),
      VALOR: String(values[4] ?? '').trim(),
    }

    if (!row.EQUIPE || !row.COD || !row.AREA || !row.FORNECEDOR || !row.VALOR) {
      errors.push(`Linha ${lineNumber}: campos obrigatorios nao preenchidos.`)
      continue
    }
...
function downloadModelFile() {
  const modelContent = [
    MOTOR_HEADERS.join(CSV_DELIMITER),
...
function downloadMetasMinimasModelFile() {
  const modelContent = [
    METAS_MINIMAS_HEADERS.join(CSV_DELIMITER),
    'EQUIPE NORTE,3231,MP,BAUDUCCO,5000',
    'EQUIPE SUL,4120,MP,BENEVIA,3500',
  ].join('\n')

  downloadCsv('modelo_metas_minimas.csv', modelContent)
}
...
export default function ConfiguracoesPage({ embedded = false }) {
  ...
  const [minimasFile, setMinimasFile] = useState(null)
  const [isImportingMinimas, setIsImportingMinimas] = useState(false)
  const [minimasError, setMinimasError] = useState('')
  const [minimasSummary, setMinimasSummary] = useState(
    metasMinimas.length > 0
      ? { rows: metasMinimas.length, fileName: 'carga anterior' }
      : null,
  )
  ...
  async function handleImportMetasMinimas() {
    if (!minimasFile) {
      setMinimasError('Selecione um arquivo de metas minimas para importar.')
      return
    }

    if (typeof setMetasMinimas !== 'function') {
      setMinimasError('Estado global de metas minimas indisponivel.')
      return
    }

    setIsImportingMinimas(true)
    setMinimasError('')

    try {
      const text = await minimasFile.text()
      const rawRows = parseMetasMinimas(text)
      if (!Array.isArray(rawRows) || rawRows.length === 0) {
        throw new Error('O arquivo nao possui linhas validas para importacao.')
      }

      setMetasMinimas(rawRows)
      setMinimasSummary({
        rows: rawRows.length,
        fileName: minimasFile.name,
      })
    } catch (e) {
      setMinimasError(e.message || 'Falha ao importar metas minimas.')
      setMinimasSummary(null)
    } finally {
      setIsImportingMinimas(false)
    }
  }
  ...
  return (
    <main className={embedded ? 'space-y-4' : 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6'}>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex items-center gap-2 mb-4">
          <FileSpreadsheet size={18} className="text-[#4997D0]" />
          <h3 className="text-lg font-bold text-slate-900">Metas Minimas</h3>
        </div>

        <p className="mb-4 text-sm text-slate-500">
          Parametro unico para todos os meses em que a meta for distribuida.
        </p>

        <div className="mb-4 flex justify-end gap-3 flex-wrap">
          <button
            type="button"
            onClick={downloadMetasMinimasModelFile}
            className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50"
          >
            <FileSpreadsheet size={16} />
            Baixar Modelo
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-4 items-end">
          <div>
            ...
            <input
              type="file"
              accept={acceptedFormats}
              onChange={(e) => setMinimasFile(e.target.files?.[0] || null)}
              className="w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm"
            />
            <p className="mt-2 text-xs text-slate-500">
              Cabecalho obrigatorio (ordem exata): EQUIPE,COD,AREA,FORNECEDOR,VALOR.
            </p>
          </div>

          <button
            type="button"
            onClick={handleImportMetasMinimas}
            disabled={isImportingMinimas}
            className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#4997D0] px-5 py-2.5 text-sm font-bold text-white hover:brightness-110 transition-all disabled:opacity-60"
          >
            <Upload size={16} />
            {isImportingMinimas ? 'Importando...' : 'Importar Metas Minimas'}
          </button>
        </div>
        ...
        {minimasSummary && (
          <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-4">
            <div className="flex items-center gap-2 text-emerald-700 mb-2">
              <CheckCircle2 size={16} />
              <p className="text-sm font-bold">Metas minimas carregadas: {minimasSummary.fileName}</p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
              <div className="rounded-md bg-white/70 px-3 py-2">
                <span className="text-slate-500">Linhas</span>
                <p className="font-bold text-slate-900">{minimasSummary.rows}</p>
              </div>
              <div className="rounded-md bg-white/70 px-3 py-2">
                <span className="text-slate-500">Em memoria</span>
                <p className="font-bold text-slate-900">{metasMinimas.length}</p>
              </div>
            </div>
          </div>
        )}
